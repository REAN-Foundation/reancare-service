...
export class AddressValidator {
    ...
    static search = async (
        ...
        await query('postalCode').optional().trim().escape().run(request);
        await query('longitudeFrom').optional().trim().escape().isDecimal().run(request);
    ...
    private static async validateBody(request) {
        ...
        await body('PersonId').optional().trim().escape().isUUID().run(request);
        await body('OrganizationId').optional().trim().escape().isUUID().run(request);
        await body('Type').optional().trim().escape().run(request);
        await body('AddressLine').exists().trim().run(request);
        await body('City').optional().trim().escape().run(request);
        await body('District').optional().trim().escape().run(request);
    ...
    private static getFilter(request): AddressSearchFilters {
        ...
        var filters: AddressSearchFilters = {
    ...
    static update = async (request: express.Request, response: express.Response): Promise<AddressDomainModel> => {
        var id = await AddressValidator.getParamId(request);
        await AddressValidator.validateBody(request);
        var domainModel = AddressValidator.getDomainModel(request);
        domainModel.id = id;
        return domainModel;
    };

...
@injectable()
export class ExerciseTaskHandler implements IUserTaskHandler {

    async processTask(userTask: UserTaskMessageDto, actionData: UserTaskActionData): Promise<ProcessedTaskDto> {
        try {
            Logger.instance().log(`Processing exercise task: ${JSON.stringify(userTask)}`);
            ...
            Logger.instance().log(`Error processing exercise task: ${error}`);
            throw error;
        }
    }

    private buildMessage(actionData: UserTaskActionData, rawContent: any): string {
        if (rawContent?.Exercise) {
            const parts = [`Exercise: ${rawContent.Exercise}`];

            if (rawContent.DurationInMin) {
                parts.push(`for ${rawContent.DurationInMin} minutes`);
            }

...
@injectable()
export class ChallengeTaskHandler implements IUserTaskHandler {

    async processTask(userTask: UserTaskMessageDto, actionData: UserTaskActionData): Promise<ProcessedTaskDto> {
        try {
            Logger.instance().log(`Processing challenge task: ${JSON.stringify(userTask)}`);
            ...
            Logger.instance().log(`Error processing challenge task: ${error}`);
            throw error;
        }
    }

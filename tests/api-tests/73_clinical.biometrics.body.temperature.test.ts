...
describe('Body temperature tests', function() {

    var agent = request.agent(infra._app);

    it('340 - Create body temperature', function(done) {
    ...
    it('341 - Get body temperature by id', function(done) {
      
        agent
            .get(`/api/v1/clinical/biometrics/body-temperatures/${getTestData('BodyTemperatureId')}`)
            .set('Content-Type', 'application/json')
            .set('x-api-key', 'T26BP24-MRGMRYE-JB90S0V-NC93PY0')
            .set('Authorization', `Bearer ${getTestData("PatientJwt")}`)
            .expect(response => {
                ...
                expect(response.body.Data.BodyTemperature).to.have.property('RecordDate');
                expect(response.body.Data.BodyTemperature).to.have.property('RecordedByUserId');
                expect(response.body.Data.BodyTemperature.id).to.equal(getTestData("BodyTemperatureId"));
            
            })
            .expect(200, done);
    });

    it('342 - Search body temperature records', function(done) {
    ...
    it('343 - Update body temperature', function(done) {
        loadBodyTemperatureUpdateModel();
        const updateModel = getTestData("BodyTemperatureUpdateModel");
        agent
            .put(`/api/v1/clinical/biometrics/body-temperatures/${getTestData('BodyTemperatureId')}`)
            .set('Content-Type', 'application/json')
            .set('x-api-key', 'T26BP24-MRGMRYE-JB90S0V-NC93PY0')
            .set('Authorization', `Bearer ${getTestData("PatientJwt")}`)
            .send(updateModel)
            .expect(response => {
                ...
                expect(response.body.Data.BodyTemperature.RecordDate).to.equal(getTestData("BodyTemperatureUpdateModel").RecordDate);
            })
            .expect(200, done);
    });

    it('344 - Delete body temperature', function(done) {
       
        agent
            .delete(`/api/v1/clinical/biometrics/body-temperatures/${getTestData('BodyTemperatureId')}`)
            .set('Content-Type', 'application/json')
            .set('x-api-key', 'T26BP24-MRGMRYE-JB90S0V-NC93PY0')
            .set('Authorization', `Bearer ${getTestData("PatientJwt")}`)
            .expect(response => {

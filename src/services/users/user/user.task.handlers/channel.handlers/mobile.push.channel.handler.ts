...
@injectable()
export class MobilePushChannelHandler implements IUserTaskChannelHandler {
    ...
    async sendMessage(userTask: UserTaskMessageDto, processedTask: ProcessedTaskDto): Promise<boolean> {
        try {
            ...
            if (!deviceTokens || deviceTokens.length === 0) {
                Logger.instance().log(`No device tokens found for user: ${userTask.UserId}`);
                return false;
            }
            ...
            Logger.instance().log(`Successfully sent MobilePush to ${deviceTokens.length} devices for task: ${userTask.id}`);
            return true;

        } catch (error) {
            Logger.instance().log(`Error sending MobilePush message: ${error}`);
            return false;
        }
    }

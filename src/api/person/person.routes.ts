...
export const register = (app: express.Application): void => {
    ...
    router.get('/phone/:phone/role/:roleId', authenticator.authenticateUser, controller.getAllPersonsWithPhoneAndRole);
    router.get('/phone/:phone', authenticator.authenticateUser, controller.getAllPersonsWithPhone);
    //Person may be associated with multiple organizations.
    //e.g. A apecialist doctor may be associated multiple hospitals
    router.get('/:id/organizations', authenticator.authenticateUser, controller.getOrganizations);

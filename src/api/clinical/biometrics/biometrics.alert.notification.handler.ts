...
///////////////////////////////////////////////////////////////////////////////
@injectable()
export class BiometricAlerNotificationHandler implements IBiometricAlert{
    ...
    private _bloodGlucoseAlertQueue = asyncLib.queue((model: BloodGlucoseAlertCreateModel, onCompleted) => {
        (async () => {
            await this.sendBloodGlucoseAlert(model);
            onCompleted();
        })();
    }, this._numAsyncTasks);
    
   private  _bloodPressureAlertQueue = asyncLib.queue(( model: BloodPressureAlertCreateModel , onCompleted) => {
       (async () => {
           await this.sendBloodPressureAlert(model);
           onCompleted();
       })();
   }, this._numAsyncTasks);

   private _pulseAlertQueue = asyncLib.queue((model: PulseAlertCreateModel, onCompleted) => {
       (async () => {
           await this.sendPulseAlert(model);
           onCompleted();
       })();
   }, this._numAsyncTasks);

   private _bodyTemperatureAlertQueue = asyncLib.queue((model: BodyTemperatureAlertCreateModel, onCompleted) => {
       (async () => {
           await this.sendBodyTemperatureAlert(model);
           onCompleted();
       })();
   }, this._numAsyncTasks);

   private _bloodOxygenAlertQueue = asyncLib.queue((model: BloodOxygenAlertCreateModel, onCompleted) => {
       (async () => {
           await this.sendBloodOxygenSaturationAlert(model);
           onCompleted();
       })();
   }, this._numAsyncTasks);

private _bodyBmiAlertQueue = asyncLib.queue((model: BodyBmiAlertCreateModel, onCompleted) => {
    (async () => {
        await this.sendBodyBmiAlert(model);
        onCompleted();
    })();
}, this._numAsyncTasks);

async bodyTemperatureAlert(model: BodyTemperatureAlertCreateModel) {
    this._bodyTemperatureAlertQueue.push(model, (err) => {
        if (err) {
...
async bloodPressureAlert(model: BloodPressureAlertCreateModel) {
    this._bloodPressureAlertQueue.push(model, (err) => {
        if (err) {
...
async bloodGlucoseAlert(model: BloodGlucoseAlertCreateModel) {
    this._bloodGlucoseAlertQueue.push(model, (err) => {
        if (err) {
...
async pulseAlert(model: PulseAlertCreateModel) {
    this._pulseAlertQueue.push(model, (err) => {
        if (err) {
...
async bloodOxygenSaturationAlert(model: BloodOxygenAlertCreateModel) {
    this._bloodOxygenAlertQueue.push(model, (err) => {
        if (err) {
...
async bmiAlert(model: BodyBmiAlertCreateModel) {
    this._bodyBmiAlertQueue.push(model, (err) => {
        if (err) {

...
@injectable()
export class BiometricsTaskHandler implements IUserTaskHandler {

    async processTask(userTask: UserTaskMessageDto, actionData: UserTaskActionData): Promise<ProcessedTaskDto> {
        try {
            Logger.instance().log(`Processing biometrics task: ${JSON.stringify(userTask)}`);
            ...
            Logger.instance().log(`Error processing biometrics task: ${error}`);
            throw error;
        }
    }

    private buildMessage(actionData: UserTaskActionData, rawContent: any): string {
        const title = actionData?.Title || 'Biometric Measurement Reminder';

        if (actionData?.Description) {
            return `${title} - ${actionData.Description}`;
        }

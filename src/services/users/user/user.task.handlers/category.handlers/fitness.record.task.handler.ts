...
@injectable()
export class FitnessRecordTaskHandler implements IUserTaskHandler {

    async processTask(userTask: UserTaskMessageDto, actionData: UserTaskActionData): Promise<ProcessedTaskDto> {
        try {
            Logger.instance().log(`Processing fitness record task: ${JSON.stringify(userTask)}`);

            const message = actionData?.Description ||
                           actionData?.Title ||
                           'Fitness Record Reminder';
            ...
            Logger.instance().log(`Error processing fitness record task: ${error}`);
            throw error;
        }
    }

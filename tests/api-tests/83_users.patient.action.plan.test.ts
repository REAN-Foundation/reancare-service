...
describe('Action plan tests', function() {
    ...
    it('385 - Create action plan', function(done) {
        ...
        agent
            ...
            .expect(response => {
                ...
                expect(response.body.Data.ActionPlan.ProviderEnrollmentId).to.equal(getTestData("ActionPlanCreateModel").ProviderEnrollmentId);
                expect(response.body.Data.ActionPlan.Provider).to.equal(getTestData("ActionPlanCreateModel").Provider);
                ...
                expect(response.body.Data.ActionPlan.ScheduledEndDate).to.equal(getTestData("ActionPlanCreateModel").ScheduledEndDate);

            })
            .expect(201, done);
    });

    it('386 - Get selected action plans', function(done) {
    ...
    it('387 - Get action plans', function(done) {
    ...
    it('388 - Search action plan records', function(done) {
    ...
    it('389 - Update action plan', function(done) {
        loadActionPlanUpdateModel();
        const updateModel = getTestData("ActionPlanUpdateModel");
        agent
            .put(`/api/v1/action-plans/${getTestData('ActionPlanId')}`)
            .set('Content-Type', 'application/json')
            .set('x-api-key', 'T26BP24-MRGMRYE-JB90S0V-NC93PY0')
            .set('Authorization', `Bearer ${getTestData("PatientJwt")}`)
            .send(updateModel)
            .expect(response => {
                ...
                expect(response.body.Data.ActionPlan.ProviderEnrollmentId).to.equal(getTestData("ActionPlanUpdateModel").ProviderEnrollmentId);
                expect(response.body.Data.ActionPlan.Provider).to.equal(getTestData("ActionPlanUpdateModel").Provider);
                ...
                expect(response.body.Data.ActionPlan.ScheduledEndDate).to.equal(getTestData("ActionPlanUpdateModel").ScheduledEndDate);

            })
            .expect(200, done);
    });

    it('390 - Delete action plan', function(done) {
        
        agent
            .delete(`/api/v1/action-plans/${getTestData('ActionPlanId')}`)
            .set('Content-Type', 'application/json')
            .set('x-api-key', 'T26BP24-MRGMRYE-JB90S0V-NC93PY0')
            .set('Authorization', `Bearer ${getTestData("PatientJwt")}`)
            .expect(response => {
...
export const loadActionPlanCreateModel = async (
    ...
    const model = {
        ...
        ScheduledEndDate     : "2021-09-12T00:00:00.000Z"
    };
...
function loadActionPlanQueryString() {
    ...
    const queryString = '?title=Monitor blood pressure on regular basis';
    return queryString;
}

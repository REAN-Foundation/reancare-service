...
@injectable()
export class PersonalReflectionTaskHandler implements IUserTaskHandler {

    async processTask(userTask: UserTaskMessageDto, actionData: UserTaskActionData): Promise<ProcessedTaskDto> {
        try {
            Logger.instance().log(`Processing personal reflection task: ${JSON.stringify(userTask)}`);
            ...
            Logger.instance().log(`Error processing personal reflection task: ${error}`);
            throw error;
        }
    }

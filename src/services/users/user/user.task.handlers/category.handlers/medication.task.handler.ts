...
@injectable()
export class MedicationTaskHandler implements IUserTaskHandler {

    async processTask(userTask: UserTaskMessageDto, actionData: UserTaskActionData): Promise<ProcessedTaskDto> {
        try {
            Logger.instance().log(`Processing medication task: ${JSON.stringify(userTask)}`);
            ...
            Logger.instance().log(`Error processing medication task: ${error}`);
            throw error;
        }
    }

    private buildMessage(actionData: UserTaskActionData, rawContent: any): string {
        if (rawContent?.DrugName) {
            const parts = [`Medication Reminder: ${rawContent.DrugName}`];

            if (rawContent.Dose && rawContent.DosageUnit) {
                parts.push(`${rawContent.Dose} ${rawContent.DosageUnit}`);
            }

            if (rawContent.Instructions) {
                parts.push(`- ${rawContent.Instructions}`);
            }

...
@injectable()
export class EducationalVideoTaskHandler implements IUserTaskHandler {

    async processTask(userTask: UserTaskMessageDto, actionData: UserTaskActionData): Promise<ProcessedTaskDto> {
        try {
            Logger.instance().log(`Processing educational video task: ${JSON.stringify(userTask)}`);
            ...
            Logger.instance().log(`Error processing educational video task: ${error}`);
            throw error;
        }
    }

    private buildMessage(actionData: UserTaskActionData, rawContent: any): string {
        const title = actionData?.Title || 'Educational Video';

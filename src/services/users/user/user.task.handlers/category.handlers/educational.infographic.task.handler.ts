...
@injectable()
export class EducationalInfographicTaskHandler implements IUserTaskHandler {

    async processTask(userTask: UserTaskMessageDto, actionData: UserTaskActionData): Promise<ProcessedTaskDto> {
        try {
            Logger.instance().log(`Processing educational infographic task: ${JSON.stringify(userTask)}`);

...
describe('Blood glucose tests', function() {

    var agent = request.agent(infra._app);

    it('355 - Create blood glucose', function(done) {
        ...
        agent
            ...
            .expect(response => {
                ...
                expect(response.body.Data.BloodGlucose.RecordDate).to.equal(getTestData("BloodGlucoseCreateModel").RecordDate);
    ...
    it('356 - Get blood glucose by id', function(done) {

        agent
            .get(`/api/v1/clinical/biometrics/blood-glucose/${getTestData('BloodGlucoseId')}`)
            .set('Content-Type', 'application/json')
            .set('x-api-key', 'T26BP24-MRGMRYE-JB90S0V-NC93PY0')
            .set('Authorization', `Bearer ${getTestData("PatientJwt")}`)
            .expect(response => {
    ...
    it('357 - Search blood glucose records', function(done) {
    ...
    it('358 - Update blood glucose', function(done) {
        ...
        agent
            ...
            .expect(response => {
                ...
                expect(response.body.Data.BloodGlucose.RecordDate).to.equal(getTestData("BloodGlucoseUpdateModel").RecordDate);

            })
            .expect(200, done);
    });

    it('359 - Delete blood glucose', function(done) {
       
        agent
            .delete(`/api/v1/clinical/biometrics/blood-glucose/${getTestData('BloodGlucoseId')}`)
            .set('Content-Type', 'application/json')
            .set('x-api-key', 'T26BP24-MRGMRYE-JB90S0V-NC93PY0')
            .set('Authorization', `Bearer ${getTestData("PatientJwt")}`)
            .expect(response => {
...
function loadBloodGlucoseQueryString() {
    ...
    const queryString = '?createdDateFrom=2021-09-12';
    return queryString;
}

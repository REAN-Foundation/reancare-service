...
@injectable()
export class EducationalLinkTaskHandler implements IUserTaskHandler {

    async processTask(userTask: UserTaskMessageDto, actionData: UserTaskActionData): Promise<ProcessedTaskDto> {
        try {
            Logger.instance().log(`Processing educational link task: ${JSON.stringify(userTask)}`);
            ...
            Logger.instance().log(`Error processing educational link task: ${error}`);
            throw error;
        }
    }

    private buildMessage(actionData: UserTaskActionData, rawContent: any): string {
        const title = actionData?.Title || 'Educational Content';

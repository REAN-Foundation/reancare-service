...
@injectable()
export class EducationalAudioTaskHandler implements IUserTaskHandler {

    async processTask(userTask: UserTaskMessageDto, actionData: UserTaskActionData): Promise<ProcessedTaskDto> {
        try {
            Logger.instance().log(`Processing educational audio task: ${JSON.stringify(userTask)}`);
            ...
            Logger.instance().log(`Error processing educational audio task: ${error}`);
            throw error;
        }
    }

    private buildMessage(actionData: UserTaskActionData, rawContent: any): string {
        const title = actionData?.Title || 'Educational Audio';

...
* This job uses [docker/build-push-action](https://github.com/marketplace/actions/build-and-push-docker-images).
* This job uses a 'dev' environment and login to ECR using creds and builds a new ECR docker image with image tag using branch name and short SHA of commit for example ``` /real care-service-dev-uat:develop_5e38e33 ```
...
2. Whenever a Branch with the prefix of 'release/' creates a pull request
...
* This job uses [docker/build-push-action](https://github.com/marketplace/actions/build-and-push-docker-images).
* This job uses the 'UAT' environment and login to ECR using creds and builds a new ECR docker image with image tag using branch name and short SHA of commit for example ``` /reancare-service-dev-uat:develop_5e38e33 ```. 
* Then it will create a new version of the Amazon ECS task definition with a new docker image and deploy the Amazon ECS task definition using Duplo API.
...
* This job uses [docker/build-push-action](https://github.com/marketplace/actions/build-and-push-docker-images).
* This job login to ECR using creds and builds a new ECR docker image with image tag using the ID of release which Publish-Release job created for example ``` reancare:97777323 ```.
* Then it will create a new version of the Amazon ECS task definition with a new docker image and deploy the Amazon ECS task definition using Duplo API.
